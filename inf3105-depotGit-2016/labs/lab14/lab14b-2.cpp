...
void lireEntreeStandard(unordered_map<string, unordered_map<string, int>>& dict);
void inserer(unordered_map<string, unordered_map<string, int>>& dict, vector<string>& phrase);
void afficher(unordered_map<string, unordered_map<string, int>>& dict);
...
void inserer(unordered_map<string, unordered_map<string, int>>& dict, vector<string>& phrase) {
  for (vector<string>::size_type i = 0; i < phrase.size(); i++) {
    ...
    for (vector<string>::size_type j = 0; j < phrase.size(); j++) {
      ...
      unordered_map<string, unordered_map<string, int>>::iterator it1 = dict.find(mot1);
      if (it1 != dict.end()) {
        unordered_map<string, int>::iterator it2 = it1->second.find(mot2);
